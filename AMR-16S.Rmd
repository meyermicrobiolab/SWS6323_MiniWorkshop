...
## Step 2: Assign taxonomy in DADA2 - this step will be completed in class 4/19/24
...
taxa <- assignTaxonomy(seqtab.nochim, "~/Documents/SWS6323_MiniWorkshop/silva_nr99_v138.1_train_set.fa.gz", multithread=TRUE)
...
ps <- phyloseq(otu_table(otu, taxa_are_rows=FALSE), 
               sample_data(samples), 
               tax_table(taxon))
ps # check that all of your samples are included: 8467 taxa and 52 samples

# use phyloseq to remove chloroplasts and mitochondria and Eukaryota
get_taxa_unique(ps, "Family") # 486
get_taxa_unique(ps, "Order") # 287
get_taxa_unique(ps, "Kingdom") # 3
ps <- subset_taxa(ps, Family !="Mitochondria")
ps <- subset_taxa(ps, Order !="Chloroplast")
ps <- subset_taxa(ps, Kingdom !="Eukaryota")
ps <- subset_taxa(ps, Kingdom !="NA")
get_taxa_unique(ps, "Family") # 483
get_taxa_unique(ps, "Order") # 285
get_taxa_unique(ps, "Kingdom") # 2
ps  #7649  taxa and 52 samples
...
# Now, time to explore the data with phyloseq.
...
This block of code will perform center-log-ratio transformation on ASVs and calculate Aitchison Distance and principal components. This is the recommended pipeline for compositional datasets. 
...
dist.clr <- dist(E.clr)
perm<-adonis2(dist.clr~type*frac,as(sample_data(ps5),"data.frame"))
print(perm)
```
...
For comparison to the ordination generated above designed for compositional datasets, let's see what an ordination using a more traditional workflow looks like - specifically an nMDS ordination from the Bray-Curtis dissimilarity of relative abundance data.
...
#pdf("nMDS.pdf",bg ="white",width=8.5)
...
sample_data(ps5_ra)$type<-factor(sample_data(ps_ra)$type, levels=c("before","after"))
...
p1=plot_bar(p5s_ra, "colony" ,fill="Order")+
  facet_grid(fraction~type,scales="free",space="free")+
